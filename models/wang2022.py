...
# Relax and focus on brain tumor segmentation
...
class wang2022(nn.Module):
    '''(https://www.sciencedirect.com/science/article/pii/S1361841521003042)

    Note: So many ERRORs in code. (https://github.com/wangpei7001/relax-and-focus-on-brain-tumor-segmentation/blob/main/models.py)
    Line 50-62: Conv3d ->> Conv3d_Block. & Line 147: y6 ->> y5 ....etc.
    # flops & params: 1.584T, 8.181M, 86.5455596446991 s
    '''
    def __init__(self, in_channel=4, out_channel=4, norm='bn', net_arch=None, net_arch_dir=None): # num_downsamples is const int 4
        super(wang2022, self).__init__()

        channels= [in_channel, 32, 64, 128, 256, 128, 64, 32, out_channel]
        padding_list=[0,4,8,12]
        dilation_list=[2,6,10,14]
        kernel_size = 3
        num_branches = 4
        self.network = relax_focus(channels, kernel_size, padding_list, dilation_list, num_branches)
...
class relax_focus(nn.Module):
    def __init__(self, channels, kernel_size,padding_list, dilation_list, num_branches):
        ...
        self.block4_1 = Conv3d_Block(self.channels[5],self.channels[6],self.channels[6])
        self.block4_2 = Conv3d_Block(self.channels[6],self.channels[7],self.channels[7])
        self.block5_1 = Conv3d_Block(self.channels[6],self.channels[7],self.channels[7])
    ...
    def forward(self, x1, y):
        ...
        x5= self.block7(torch.cat((x4,x1),1))
        x5 = self.auto7(x5)
        x5 = self.block8(torch.cat((x5,y5,y5,y4),dim = 1))
        x5 = self.fc(x5)
        return x4
...
class Autofocus_single(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(1, self.num_branches):
            ...
            # x1 = x1 + x2* att[:,i:(i+1),:,:,:].expand(shape)
